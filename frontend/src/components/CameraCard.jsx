...
export default function CameraCard({ camera, models, onAssignModel, onStart, onStop, onDelete, delay = 0 }) {
  ...
  const statusConfig = {
    running: { color: 'text-emerald-600', bg: 'bg-emerald-100', label: 'Detecting' },
    started: { color: 'text-emerald-600', bg: 'bg-emerald-100', label: 'Started' },
    detected: { color: 'text-blue-600', bg: 'bg-blue-100', label: 'Object Found' },
    scanning: { color: 'text-cyan-600', bg: 'bg-cyan-100', label: 'Scanning' },
    stopped: { color: 'text-gray-500', bg: 'bg-gray-100', label: 'Stopped' },
    idle: { color: 'text-gray-400', bg: 'bg-gray-50', label: 'Idle' },
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        className="relative w-full aspect-video bg-gray-100 rounded-xl mb-4 overflow-hidden flex items-center justify-center"
      >
        ...
        {(camera.sourceType === 'file' || camera.sourceType === 'sample') ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">{camera.sourceType} Feed</span>
          </div>
        )}
        ...
        {camera.isDetecting && (
          <div className="absolute top-2 left-2 flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider bg-white/80 px-1.5 py-0.5 rounded">LIVE</span>
          </div>
        )}

        {/* FPS badge */}
        {fps > 0 && (
          <div className="absolute top-2 right-2 bg-gray-800/70 text-green-400 text-[10px] font-mono font-bold px-2 py-0.5 rounded-md">
            {fps} FPS
          </div>
        )}
        ...
        <div className="absolute bottom-2 left-2">
          {camera.isOnline ? (
            <div className="flex items-center gap-1 text-[10px] text-emerald-600 bg-white/80 px-1.5 py-0.5 rounded">
              <Wifi className="w-3 h-3" /> Online
            </div>
          ) : (
            <div className="flex items-center gap-1 text-[10px] text-red-500 bg-white/80 px-1.5 py-0.5 rounded">
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="font-semibold text-gray-800 text-sm">{camera.name}</h3>
          <span className="text-[10px] text-gray-400 uppercase tracking-wider">{camera.sourceType}</span>
        </div>
      ...
      {/* Model assignment */}
      <div className="mb-3">
        <label className="text-[10px] text-gray-400 uppercase tracking-wider mb-1 block">Assigned Model</label>
        <select
