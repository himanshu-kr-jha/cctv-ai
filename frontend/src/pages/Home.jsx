  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-purple-400/5 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/3" />
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-8">
            ...
            <div className="flex-1 space-y-5">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-accent flex items-center justify-center shadow-xl shadow-accent/25 animate-float">
                  <Shield className="w-6 h-6 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-gray-800 tracking-tight">WatchTowerAI</h1>
                  <p className="text-xs text-accent/70 tracking-widest uppercase font-semibold">Intelligent Surveillance Platform</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {['ONNX Runtime', 'Socket.IO', 'BullMQ', 'React', 'MongoDB'].map((t) => (
                  <span key={t} className="text-[10px] font-semibold text-gray-500 bg-gray-100 border border-gray-200 px-2.5 py-1 rounded-lg uppercase tracking-wider">{t}</span>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
              {features.map((f, i) => (
                <motion.div
                  ...
                  className="bg-gray-50 border border-gray-200 rounded-xl p-4 hover:bg-white hover:shadow-md hover:border-gray-300 transition-all group"
                >
                  <f.icon className="w-5 h-5 text-accent mb-2 group-hover:text-accent-dark transition-colors" />
                  <h4 className="text-xs font-semibold text-gray-700 mb-1">{f.title}</h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        >
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Top Detected Labels</h3>
          {labelData.length > 0 ? (
          ...
          {stats?.recentDetections?.length > 0 ? (
            stats.recentDetections.map((det, i) => (
              <div key={i} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${det.objectsDetected > 0 ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                  <div>
                    <p className="text-sm text-gray-700 font-medium">{det.camera?.name || 'Camera'}</p>
                    <p className="text-xs text-gray-400">{det.labels?.join(', ') || 'No objects'}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-gray-600">{det.objectsDetected} objects</p>
                  <p className="text-[10px] text-gray-400">{timeAgo(det.createdAt)}</p>
                </div>
              </div>
            ))
          ) : (
